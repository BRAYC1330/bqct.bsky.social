...
class LinkExtractor:
    ...
    async def extract(self, url: str) -> Optional[str]:
        ...
        try:
            ...
            r = await self._client.get(url, headers={"User-Agent": "Mozilla/5.0"}, timeout=config.REQUEST_TIMEOUT)
            if r.status_code == 200:
                content = trafilatura_extract(r.text, include_tables=False, include_comments=False, output_format="txt")
                result = utils.sanitize_input(content[:self._max_content], for_prompt=True) if content else None
                async with self._lock:
