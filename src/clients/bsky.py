...
from src.clients import httpx_base

logger = logging.getLogger(__name__)

async def login_with_cache(handle, password):
...
async def _fetch_url_content(client, url):
    try:
        ...
        parsed = httpx_base.get_client().base_url or url
        if url.split("/")[2] not in config.ALLOWED_LINK_DOMAINS: return ""
