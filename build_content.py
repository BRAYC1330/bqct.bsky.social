...
def get_no_data_response(keyword: str) -> str:
    return f'No data found for "{keyword}". Try rephrasing your query in a new comment or DYOR.\nQwen'
...
async def build_digest(llm, trends, task_type: str, max_total: int = 300) -> str:
    ...
    if task_type == "digest_mini":
        ...
        for item in trends[:6]:
            ...
            line = f"{e} {kw} 📊 {sc}"
            if current_len + len(line) + 1 > max_body: break
            lines.append(line)
    ...
    allowed_desc = max_body - len(header) - len(title) - 2
    ...
    desc = generator.get_answer(llm, "", prompt, max_chars=allowed_desc, temperature=0.5).strip()
    desc = utils.truncate_response(desc, allowed_desc)
    if not desc or len(desc) < 5:
        desc = utils.truncate_response(summary, allowed_desc)
    return f"{header}{title} {desc}\n" + sig
