...
async def process(client, llm, task):
    ...
    target_uri = chain.get("parent_uri", uri)
    parent_cid = chain.get("parent_cid", "")
    if not parent_cid:
        logger.error("[community] Missing parent_cid, cannot post reply")
        return
    ...
    if not search_data:
        ...
        await bsky.post_reply(client, config.BOT_DID, reply, root_uri, root_cid, target_uri, parent_cid)
        return
