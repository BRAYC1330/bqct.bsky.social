...
async def process(client, llm, task):
    ...
    thread_context = utils._format_thread_for_llm(chain, config.OWNER_DID, config.BOT_DID, max_recent=8)
    final_ctx = state.merge_contexts("", thread_context, search_data, user_text)
