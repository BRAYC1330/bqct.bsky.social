...
async def _format_thread_for_llm(chain: dict, owner_did: str, bot_did: str, client: httpx.AsyncClient, max_recent: int = 20) -> str:
    ...
    root_text_raw = chain.get("root_text", "")
    if not root_text_raw and posts:
        root_text_raw = posts[0].get("record", {}).get("text", "")
    root = clean_for_llm(root_text_raw)
